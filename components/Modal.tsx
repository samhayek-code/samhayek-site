...
function getEmbedUrl(url: string): {
  src: string;
  type: "spotify" | "youtube" | "audius" | "other";
} {
  if (!url) return { src: "", type: "other" };

  // Already an embed URL
  if (url.includes("/embed/")) {
    const type = url.includes("youtube")
      ? "youtube"
  ...
  if (url.includes("youtube.com") || url.includes("youtu.be")) {
    ...
    if (url.includes("youtube.com/watch")) {
      ...
      try {
        videoId =
          new URL(url, "https://www.youtube.com").searchParams.get("v") || "";
      } catch {
        videoId = "";
      }
...
export default function Modal({ item, onClose }: ModalProps) {
  ...
  const isCollection =
    item.collectionPieces && item.collectionPieces.length > 0;
  ...
  const isGalleryType =
    ...
    !isTestimonial &&
    !isReferences &&
    !isCaseStudy; // Types that use full-width gallery display (but not collections, testimonials, case studies, or references)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`overflow-y-auto overflow-x-hidden ${showWhopCheckout ? "max-h-[95vh]" : "max-h-[85vh]"}`}
        >
          ...
          {!isEmbedModal &&
            ...
            item.coverImage && (
              <div
                className="w-full aspect-video flex items-center justify-center relative overflow-hidden img-outline"
          ...
          <div className="p-5 lg:p-8 overflow-x-hidden">
            ...
            {isCaseStudy &&
              (() => {
                const overviewSection = item.caseStudySections!.filter(
                  (s) => s.sectionTitle === "Overview"
                );
                const remainingSections = item.caseStudySections!.filter(
                  (s) => s.sectionTitle !== "Overview"
                );
                ...
                return (
                  <>
                    ...
                    {/* Video at top. No forced aspect ratio — the player uses the
                        asset's own, so a non-16:9 capture (e.g. a tall page
                        scroll-through) fills the frame instead of sitting in
                        black pillars. */}
            ...
            {!isCollection &&
              ...
              !isCaseStudy && (
                <p
                  className="font-sans text-[17px] leading-relaxed mb-6"
            ...
            {isOasis && (
              <div className="space-y-6 mb-8">
                ...
                <p
                  ...
                  style={{ color: "var(--modal-text-secondary)" }}
                >
                  Oasis is a small script that runs automatically every night at
                  midnight.
                </p>
                <p
                  className="font-sans text-[17px] leading-relaxed"
                  style={{ color: "var(--modal-text-secondary)" }}
                >
                  It takes any new files in your Downloads folder and sorts them
                  into folders by date and type — images in one place, documents
                  in another.
                </p>
                <p
                  className="font-sans text-[17px] leading-relaxed"
                  style={{ color: "var(--modal-text-secondary)" }}
                >
                  At the end of each week, those daily folders get grouped
                  together. At the end of each month, the weeks get archived.
                ...
                <div className="space-y-3">
                  ...
                  >
                    <code
                      className="font-mono text-[13px] text-foreground truncate flex-1 select-all"
                      title="curl -fsSL https://raw.githubusercontent.com/samhayek-code/OASIS/main/install.sh | bash"
                    >
                      curl -fsSL
                      https://raw.githubusercontent.com/samhayek-code/OASIS/main/install.sh
                      | bash
                    </code>
                    <button
                      onClick={() =>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <li className="flex gap-3">
                      ...
                      <span>
                        Open Terminal (search &ldquo;Terminal&rdquo; in
                        Spotlight)
                      </span>
                    </li>
                    <li className="flex gap-3">
                  ...
                  <div className="pl-4 space-y-0.5">
                    ...
                    <div className="pl-4 space-y-0.5">
                      ...
                      <div className="pl-4 space-y-0.5">
                        ...
                        <div className="pl-4 text-subtle">
            ...
            {isAudioHooks && (
              <div className="space-y-6 mb-8">
                ...
                <p
                  ...
                  style={{ color: "var(--modal-text-secondary)" }}
                >
                  Claude Code greets every session, announces your wins, calls
                  for orders when it needs you, and sounds off when something
                  breaks — so the work never feels quiet. Eight voices to choose
                  from, switch whenever.{" "}
                  <span style={{ color: "var(--modal-text-tertiary)" }}>
                    macOS only.
                  </span>
                </p>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <code
                      className="font-mono text-[13px] text-foreground truncate flex-1 select-all"
                      title="bash <(curl -fsSL https://raw.githubusercontent.com/samhayek-code/claude-audio-hooks/main/install.sh)"
                    >
                      bash &lt;(curl -fsSL
                      https://raw.githubusercontent.com/samhayek-code/claude-audio-hooks/main/install.sh)
                    </code>
                    <button
                      onClick={() =>
                        handleCopyInstall(
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <li className="flex gap-3">
                      ...
                      <span>
                        Start a new Claude Code session and you&rsquo;ll hear it
                      </span>
                    </li>
                  </ol>
                </div>
                ...
                <p
                  ...
                  style={{ color: "var(--modal-text-secondary)" }}
                >
                  Three franchises — StarCraft 2 (Terran, Protoss, Zerg), Halo
                  (Cortana, 343 Guilty Spark, Sgt. Johnson), and Tiberian Sun
                  (GDI&rsquo;s EVA, Nod&rsquo;s CABAL). Around 330 lines, picked
                  at random; error sounds are smart-filtered so routine failures
                  stay quiet.
                </p>

                {/* Packs table */}
                <div
                  className="rounded-input overflow-hidden"
                  style={{
                    border: "1px solid var(--border)",
                    background: "var(--surface)",
                  }}
                    ...
                    <table className="w-full border-collapse text-[12px]">
                      <thead>
                        <tr style={{ borderBottom: "1px solid var(--border)" }}>
                          <th
                            className="text-left font-mono font-medium uppercase tracking-wide px-3 py-2"
                          ...
                          <th
                            className="text-right font-mono font-medium uppercase tracking-wide px-3 py-2"
                            style={{ color: "var(--muted)" }}
                          >
                            Clips
                          </th>
                      ...
                      <tbody>
                        {[
                          [
                            "terran",
                            "StarCraft 2",
                            "Terran advisor + units",
                            "28",
                          ],
                          [
                            "protoss",
                            "StarCraft 2",
                            "Protoss advisor + units",
                            "23",
                          ],
                          [
                            "zerg",
                            "StarCraft 2",
                            "Zerg advisor + creature sounds",
                            "24",
                          ],
                          ...
                          >
                            <td
                              className="font-mono px-3 py-2 whitespace-nowrap"
                              style={{ color: "var(--modal-text-secondary)" }}
                            >
                              {pack}
                            </td>
                            <td
                              className="font-sans px-3 py-2 whitespace-nowrap"
                              style={{ color: "var(--modal-text-tertiary)" }}
                            >
                              {franchise}
                            </td>
                            <td
                              className="font-sans px-3 py-2"
                              style={{ color: "var(--modal-text-tertiary)" }}
                            >
                              {voices}
                            </td>
                            <td
                              className="font-mono tabular-nums text-right px-3 py-2"
                              style={{ color: "var(--modal-text-secondary)" }}
                            >
                              {clips}
                            </td>
                ...
                <p
                  ...
                  style={{ color: "var(--modal-text-secondary)" }}
                >
                  The clips ship clean. Soundboard rips came with an in-game
                  ping and a spoken watermark before each line — a numpy +
                  ffmpeg pipeline finds the ping by cross-correlation and trims
                  to the voice onset, never clipping the first word. The
                  Tiberian Sun set is the original game audio, pulled on macOS
                  by cracking Westwood&rsquo;s MIX format by hand.
                </p>
                ...
                >
                  ...
                  <div className="pl-4 space-y-0.5">
                    <div>
                      <span className="text-muted">Session starts →</span>{" "}
                      {'"Battlecruiser operational"'}
                    </div>
                    <div>
                      <span className="text-muted">Task completes →</span>{" "}
                      {'"Construction complete"'}
                    </div>
                    <div>
                      <span className="text-muted">Needs permission →</span>{" "}
                      {'"Awaiting orders"'}
                    </div>
                    <div>
                      <span className="text-muted">Error occurs →</span>{" "}
                      {'"Not enough minerals"'}
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <code
                    ...
                    style={{ color: "var(--modal-text-secondary)" }}
                  >
                    set-faction.sh cortana
                  </code>{" "}
                  (or any of the eight packs), or just ask Claude to switch it
                  for you.
                </p>
                ...
                >
                  ...
                  <code
                    ...
                    style={{ color: "var(--modal-text-secondary)" }}
                  >
                    ~/.claude/sounds/&lt;pack&gt;/&lt;event&gt;/
                  </code>
                  , or add a whole new pack as its own folder. The picker finds
                  them automatically.
                </p>
              </div>
            )}
              ...
              <div className="space-y-8 mb-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <li className="flex gap-2">
                        ...
                        <span>
                          <strong className="text-foreground">Arcarae</strong> —
                          Brand expansion and visual design for an AI
                          self-reflection app
                        </span>
                      </li>
                      <li className="flex gap-2">
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-1">
                      <h4 className="font-sans text-[16px] font-medium text-foreground">
                        Designer & Community Lead · Web3 Music Platforms
                      </h4>
                      <span className="font-mono text-[12px] text-muted shrink-0">
                        2021 – 2022
                      </span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[14px]">
                    ...
                    <div>
                      ...
                      <span className="text-muted"> — </span>
                      <span style={{ color: "var(--modal-text-secondary)" }}>
                        HTML, CSS, JavaScript, React, Next.js, Three.js, WGSL,
                        Motion
                      </span>
                    </div>
                    <div>
              ...
              (() => {
                ...
                if (item.coverImage && item.slug?.current !== "pro2tect") {
                  allImages.push({
                    ...
                    alt: item.title,
                    thumbnail: urlFor(item.coverImage)
                      .width(1600)
                      .quality(85)
                      .url(),
                  });
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-white/0 group-hover:bg-white/5 transition-colors flex items-center justify-center">
                          <span className="opacity-0 group-hover:opacity-100 transition-opacity text-white/60 text-sm font-mono">
                            {allImages.length > 1
                              ? `${i + 1} / ${allImages.length} — Click to expand`
                              : "Click to expand"}
                          </span>
                        </div>
                      </div>
                ...
                >
                  ...
                  {displayedIndex === -1 && (
                    <div className="text-center py-4">
                      <p
                        className="font-sans text-[17px] leading-relaxed max-w-lg mx-auto"
                        style={{ color: "var(--modal-text-secondary)" }}
                      >
                        {item.description}
                      </p>
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="flex items-center justify-between py-4 border-y border-border">
                            ...
                            <button
                              ...
                              disabled={
                                collectionIndex === totalPieces - 1 && !hasMerch
                              }
                              className="flex items-center gap-2 px-3 py-1.5 font-sans text-sm text-muted hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                            >
                              {collectionIndex === totalPieces - 1 && hasMerch
                          ...
                          {piece.poemText && (
                            <div className="max-w-md mx-auto">
                              <p
                                className="font-sans text-[17px] leading-[1.8] whitespace-pre-line text-center"
                                style={{ color: "var(--modal-text-tertiary)" }}
                              >
                                {piece.poemText}
                              </p>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {item.merchGallery!.map((image, i) => (
                          <div
                            key={i}
                            className="aspect-square relative overflow-hidden rounded-card img-outline"
                            style={{ background: "var(--modal-surface)" }}
                          >
                            <Image
            ...
            {isSupport && (
              <div className="relative space-y-3 mb-8">
                <WalletButton
                  ...
                  onShowQR={(currency, address) =>
                    setActiveQR(
                      activeQR?.currency === currency
                        ? null
                        : { currency, address }
                    )
                  }
            ...
            {showWhopCheckout && item.whopPlanId && (
              <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-display text-[20px] font-medium tracking-[-0.02em] text-foreground">
                    Checkout
                  </h3>
                  <button
